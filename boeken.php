...
session_start();
include 'connect.php';
if(!isset($_SESSION['user_id'])){header("Location: inloggen.php");exit;}
if(!isset($_GET['id'])||!is_numeric($_GET['id'])){echo"Geen reis geselecteerd.";exit;}
$reis_id=$_GET['id'];
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $extra=0;
    if($extra_bagage)$extra+=20*$aantal_personen;
    if($reisverzekering)$extra+=15*$aantal_personen;
    if($annulering)$extra+=10*$aantal_personen;
...
<p><?php echo htmlspecialchars($reis['locatie']);?> – <a href="<?php echo $reis['locatie_link'];?>" target="_blank" class="toon-op-kaart">Toon op kaart</a></p>
</div>
<div class="foto-grid">
<?php if(!empty($reis['afbeelding'])):?><img src="images/<?php echo $reis['afbeelding'];?>"><?php endif;?>
</div>
...
<?php if($boodschap):?><div class="success"><?php echo $boodschap;?></div><?php endif;?>
...
<label><input type="checkbox" name="extra_bagage" id="bagage" value="1"> 🧳 Extra bagage (€20 p.p.)</label><br>
<label><input type="checkbox" name="reisverzekering" id="verzekering" value="1"> 🛡️ Reisverzekering (€15 p.p.)</label><br>
<label><input type="checkbox" name="annulering" id="annulering" value="1"> ❌ Annuleringsverzekering (€10 p.p.)</label>
</fieldset>
