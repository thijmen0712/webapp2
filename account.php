        ...
        <div class="account-col reizen-lijst">
            ...
            <?php if (count($reizen) > 0): ?>
                <ul>
                    ...
                    ?>
                        <li>
                            <strong><?= htmlspecialchars($reis['titel']) ?></strong> – <?= htmlspecialchars($reis['locatie']) ?><br>
                            Boekdatum: <?= htmlspecialchars($reis['boekdatum']) ?><br>
                            Vertrekdatum: <?= htmlspecialchars($reis['vertrekdatum']) ?><br>
                            Status: <?= htmlspecialchars($reis['status']) ?><br>
                            <?php if ($magAnnuleren): ?>
                                <form method="post" action="annuleer.php" style="display:inline;">
                                    <input type="hidden" name="reis_boeking_id" value="<?= $reis['reis_boeking_id'] ?>" />
                                    <button type="submit">Annuleer</button>
                                </form>
                            <?php else: ?>
                                <em>Niet meer te annuleren</em>
                            <?php endif; ?>
                            <form method="post" action="recensie_toevoegen.php" style="margin-top:10px;">
                                <input type="hidden" name="reis_id" value="<?= $reis['reis_id'] ?>" />
                                <label>Beoordeling:
